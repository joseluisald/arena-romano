...
export const WhatsAppImportModal: React.FC<WhatsAppImportModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-sm overflow-y-auto">
      <div className="relative w-full max-w-2xl bg-[#111116] rounded-2xl shadow-2xl border border-[#272736] overflow-hidden my-auto max-h-[92vh] flex flex-col text-slate-100">
        {/* Header */}
        <div className="bg-[#161622] text-white px-5 py-4 flex items-center justify-between border-b border-[#262638]">
          <div className="flex items-center gap-2.5">
            <div className="p-2 rounded-lg bg-[#25D366] text-white shadow-md shadow-emerald-500/20">
              <MessageSquareShare size={20} />
            </div>
            <div>
              <h2 className="text-base sm:text-lg font-bold tracking-tight text-white">Importar Lista do WhatsApp</h2>
              <p className="text-xs text-slate-400">Cole a mensagem do grupo da Arena Romano</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-5">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Cole aqui a mensagem com a lista de jogadores do WhatsApp..."
              className="w-full text-xs sm:text-sm font-mono p-3 rounded-xl border border-[#262638] focus:border-[#f27d26] bg-[#0c0c11] text-white placeholder:text-slate-500 transition-all outline-none"
            />
          </div>

          {/* Step 2: Parsed Preview & Adjustments */}
          {rawText.trim().length > 0 && (
            <div className="space-y-4 pt-3 border-t border-[#262638]">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-bold px-2.5 py-0.5 rounded-full bg-orange-500/15 text-orange-300 border border-orange-500/30">
                  {selectedCount} jogadores detectados
                </span>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="sm:col-span-3">
                  <label className="text-xs font-medium text-slate-300 block mb-1">Nome do Jogo / Treino</label>
                  <input
                    type="text"
                    value={title}
                    onChange={e => setTitle(e.target.value)}
                    className="w-full px-3 py-2 text-sm font-semibold rounded-lg border border-[#262638] bg-[#0c0c11] text-white focus:border-[#f27d26] outline-none"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-1.5 text-sm rounded-lg border border-[#262638] bg-[#0c0c11] text-white focus:border-[#f27d26] outline-none font-medium"
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-8 pr-3 py-1.5 text-sm rounded-lg border border-[#262638] bg-[#0c0c11] focus:border-[#f27d26] outline-none font-bold text-[#f27d26]"
                    />
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="max-h-56 overflow-y-auto border border-[#22222c] rounded-xl p-2 bg-[#0c0c11] space-y-1.5">
                  {players.map((player, idx) => (
                    <div
                      key={player.id}
                      className={`flex items-center justify-between gap-2 p-2 rounded-lg transition-colors border ${
                    ...
                    >
                      <div className="flex items-center gap-2 flex-1 min-w-0">
                        ...
                        />
                        <span className="text-xs font-mono font-bold text-orange-400/80 w-5 text-right">
                          {idx + 1}.
                        </span>
                        <input
                          type="text"
                          ...
                          className="text-xs sm:text-sm font-medium text-white bg-transparent border-b border-transparent focus:border-[#35354a] focus:bg-[#161622] px-1 py-0.5 rounded outline-none flex-1"
                        />
                        {player.raw_tag && (
                ...
                <div className="flex items-center gap-2 mt-2">
                  <input
                    ...
                    className="text-xs sm:text-sm px-3 py-1.5 rounded-lg border border-[#262638] bg-[#0c0c11] text-white placeholder:text-slate-500 focus:border-[#f27d26] outline-none flex-1"
                  />
                  <button
                    type="button"
                    onClick={handleAddManualPlayer}
                    className="px-3 py-1.5 rounded-lg bg-[#1c1c28] hover:bg-[#28283a] text-slate-200 border border-[#2e2e42] text-xs font-bold flex items-center gap-1 cursor-pointer"
                  >
        ...
        {/* Footer Actions */}
        <div className="bg-[#161622] px-5 py-3.5 border-t border-[#262638] flex items-center justify-end gap-2">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 rounded-xl text-slate-300 hover:text-white font-semibold text-xs sm:text-sm hover:bg-[#1c1c28] transition-colors cursor-pointer border border-[#262638]"
          >
            Cancelar
          </button>
          <button
            ...
            className="px-5 py-2.5 rounded-xl bg-[#f27d26] hover:bg-[#ff8a3d] text-white font-bold text-xs sm:text-sm shadow-md shadow-orange-500/20 transition-all active-press disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer flex items-center gap-2"
          >
            <Check size={16} />
            Importar Jogo ({selectedCount} Jogadores)
          </button>
        </div>
      </div>
    </div>

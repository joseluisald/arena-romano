    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-sm overflow-y-auto">
      <div className="relative w-full max-w-xl bg-[#111116] rounded-2xl shadow-2xl border border-[#272736] overflow-hidden my-auto max-h-[92vh] flex flex-col text-slate-100">
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 overflow-y-auto space-y-4 flex-1">
          ...
          <div>
            <label className="text-xs font-bold text-slate-300 block mb-1">Nome do Jogo / Treino *</label>
            <input
              type="text"
              required
              placeholder="Ex: Churrasco do Bigode, Pelada da Terça"
              value={title}
              onChange={e => setTitle(e.target.value)}
              className="w-full px-3.5 py-2 text-sm font-semibold rounded-xl border border-[#262638] bg-[#0c0c11] text-white placeholder:text-slate-500 focus:border-[#f27d26] outline-none"
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-1.5 text-sm rounded-lg border border-[#262638] focus:border-[#f27d26] outline-none font-medium bg-[#0c0c11] text-white"
              >
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-8 pr-3 py-1.5 text-sm font-bold text-[#f27d26] rounded-lg border border-[#262638] bg-[#0c0c11] focus:border-[#f27d26] outline-none"
                />
              </div>
            </div>

            <div>
              <label className="text-xs font-medium text-slate-300 block mb-1">Observações / Recados</label>
              <input
                type="text"
                placeholder="Ex: Aniversário, levar colete..."
                value={notes}
                onChange={e => setNotes(e.target.value)}
                className="w-full px-3 py-1.5 text-sm rounded-lg border border-[#262638] bg-[#0c0c11] text-white placeholder:text-slate-500 focus:border-[#f27d26] outline-none"
              />
            </div>
          </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-xs font-bold text-slate-300 flex items-center gap-1.5">
                  <Users size={14} className="text-orange-400" /> Jogadores Iniciais ({players.length})
                </label>
              </div>
              ...
              <div className="max-h-36 overflow-y-auto border border-[#22222c] rounded-xl p-2 bg-[#0c0c11] space-y-1">
                {players.map((p, idx) => (
                  <div key={idx} className="flex items-center justify-between p-1.5 bg-[#161622] rounded border border-[#262638] text-xs">
                ...
                {players.length === 0 && (
                  <p className="text-center text-xs text-slate-500 py-2">Nenhum jogador adicionado ainda.</p>
                )}
              </div>
            </div>
          )}
